...
function Question() {
  ...
  return (
    <Qcontainer>
      ...
      <>
      <QuestionHeader title={data.title} createdAt={timeCal(data.createdAt)} modifiedAt={timeCal(data.modifiedAt)} view={data.viewCount} />
      <QContents>
        <QContentContainer>
...
export function timeCal(par){
  ...
  let result = "";
  let dif = 0;
  if(part.getFullYear() === now.getFullYear()){ // same Year
    if(part.getMonth() === now.getMonth()){ // same Month
      if(part.getDay() === now.getDay()){ // same Day
        if(part.getHours() === now.getHours()){ // same Hour
          if(part.getMinutes() === now.getMinutes()){ // same Min => NOW
            result = "지금"
          }else{ // dif Min => 몇분전
